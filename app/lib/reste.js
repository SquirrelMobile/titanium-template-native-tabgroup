/**
 * @class Lib.reste https://github.com/jasonkneen/RESTe
 * RESTe library to handle HTTP request
 */
var main = function() {
  var reste = this;
  ...
  reste.getLastAction = function() {
    return {
      lastArgs: lastArgs,
      lastOnLoad: lastOnLoad,
      lastOnError: lastOnError
    };
  };

  reste.launchLastAction = function(args, onLoad, onError) {
  ...
  function makeHttpRequest(args, onLoad, onError) {
    ...
    requestHeaders.forEach(function(header) {
      ...
      http.setRequestHeader(
        header.name,
        typeof header.value == "function" ? header.value : header.value
      );
    });
      ...
      if (args.collections) {
        args.collections.forEach(function(collection) {
          Alloy.Collections[collection.name] =
            Alloy.Collections[collection.name] || new Backbone.Collection();
